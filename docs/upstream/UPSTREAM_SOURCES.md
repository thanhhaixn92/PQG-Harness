...
To minimize reimplementation risk, the PR keeps the already-reviewed PQG workspace and MCP implementations byte-for-byte as internal base modules, then places the runner-owned cancellation adapter around their command seam:
...
- `agents/_workspace.ts` and `agents/_mcp-bridge.ts` are thin adapters; the only new platform behavior lives in `agents/_sandbox-abort.ts` and the simplified `agents/stop.ts`.
